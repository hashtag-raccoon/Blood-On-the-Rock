...
public class MultiCharacterRigSetup : EditorWindow
{
    ...
    {
        // 몸 크기를 기준으로 상대 오프셋 계산
        Vector2 bodySize = Vector2.one;
        if (sprites.ContainsKey("body"))
        {
            bodySize = sprites["body"].bounds.size;
        }
        float headYOffset = bodySize.y * 0.25f;
        float armXOffset = bodySize.x * 0.18f;
        float armYOffset = bodySize.y * 0.10f;
        float legXOffset = bodySize.x * 0.09f;
        float legYOffset = bodySize.y * 0.30f;
        ...
        headParent.transform.localPosition = new Vector3(0, headYOffset, 0);
