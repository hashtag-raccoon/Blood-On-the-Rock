...
public class ArbeitPoint : MonoBehaviour
{
    ...
    public Vector3 CalculateWaitingPosition(int position)
    {
        ...
        // 스크린 상 일반적인 상하좌우 방향으로 오프셋 계산
        // 아이소메트릭 타일맵 위치에 맞게 조정됨
        switch (lineDirection)
        {
            case IsometricLineDirection.Up:
                offset = new Vector3(0, position * spacing, 0);
                break;
            ...
            case IsometricLineDirection.Left:
                offset = new Vector3(-position * spacing, 0, 0);
                break;
        ...
        return basePosition + offset; // 대기 위치에 오프셋 더하기, 오프셋은 스크린 상 방향기준으로 계산됨
    }
