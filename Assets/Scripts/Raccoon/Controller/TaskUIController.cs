...
public class TaskUIController : MonoBehaviour
{
    ...
    [SerializeField] private UnityEngine.UI.Image taskIcon; // TaskType에 따라 변경될 아이콘
    ...
    private void Awake()
    {
        ...
        if (taskIcon == null)
            taskIcon = GetComponentInChildren<UnityEngine.UI.Image>();
    }
    ...
    /// <summary>
    /// 손님/테이블용 UI 초기화 (업무 타입 포함)
    /// </summary>
    public void InitializeTargetUI(GameObject target, TaskType taskType, Vector3 offset, Vector2 uiSize, Vector3 scale)
    {
        targetObject = target;
        isArbeitUI = false;
    ...
    private void UpdateTaskIcon(TaskType taskType)
    {
        if (taskIcon == null)
        {
            Debug.LogWarning("TaskIcon Image가 할당되지 않았습니다.");
            return;
        }
